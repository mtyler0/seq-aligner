...
def get_aa_matrix(matrix_file):
    ...
    Args:
    matrix_file (str): Path to blosum62 or pam160 matrix for protein alignments
    
    Returns:
        dict: nested dictionary scoring matrix
    """
...
def get_fasta_seq(fasta_file):
    ...
    Args:
        fasta_file (str): Path to the uploaded file from src/data/uploaded_files

    Returns:
        str: Nucleotide/amino acid sequence from file contents
    """
...
def format_alignment(seq1, matches, seq2, line_length=60, start1=1, start2=1):
    ...
    Args:
        seq1 (str): subject sequence (top) output from alignment algo
        matches (str): Match identifier; "|" for match, "." for mismatch
        seq2 (str): Query sequence (bottom) output from alignment algo
        line_length (int, optional): Max length of letters in one line. Defaults to 60
        start1 (int, optional): _description_. Defaults to 1
        start2 (int, optional): _description_. Defaults to 1

    Returns:
        str: Formatted string of aligned sequences in BLAST format
    """
    output_lines = []
